...
if (_UnitGroup getVariable ["VCM_TOUGHSQUAD",false]) exitWith {};
if (_UnitGroup getVariable ["Vcm_Disable",false]) exitWith {};
...
{
	{
		if (_RespondCount < _EnemyCount) then
		{
			...
			if (!(isNil "_CheckStatus2") && {!(_x getVariable ["Vcm_Disable",false])} && {!(_x getVariable ["VCM_NOFLANK",false])}  && {!(_x getVariable ["VCM_MOVE2SUP",false])} && {"ItemRadio" in _CheckStatus2}) then 
			{
