...
private _WayPointType = selectRandom ["Assault","High","Low","Retreat","Flank"];
    ...
    case "High": 
		{
			...
			private _waypoint0 = _group addwaypoint [_HighP,0];
			_waypoint0 setwaypointtype "MOVE";
			_waypoint0 setWaypointSpeed "FULL";
			_group setCurrentWaypoint [_group,(_waypoint0 select 1)];			
			private _waypoint0 = _group addwaypoint [(getpos _NearestEnemy),0];
    ...
    case "Retreat": 
		{
			...
			private _waypoint0 = _group addwaypoint [_MovePosition,0];
			_waypoint0 setwaypointtype "MOVE";
			_waypoint0 setWaypointSpeed "FULL";
			_group setCurrentWaypoint [_group,(_waypoint0 select 1)];			
			private _waypoint0 = _group addwaypoint [_MovePosition,0];
    ...
    case "FlankL": 
		{
			...
			private _dir = random 360;
			private _positions = [(_myEnemyPos select 0) + (sin _dir) * _dist, (_myEnemyPos select 1) + (cos _dir) * _dist, 0];
			private _positionsL = [(_Leader select 0) + (sin _dir) * _dist, (_Leader select 1) + (cos _dir) * _dist, 0];
			private _myPlaces = selectBestPlaces [_myEnemyPos, 250,"((6*hills + 2*forest + 4*houses + 2*meadow) - sea + (2*trees)) - (1000*deadbody)", 100, 5];
			private _myPlacesL = selectBestPlaces [_leader, 250,"((6*hills + 2*forest + 4*houses + 2*meadow) - sea + (2*trees)) - (1000*deadbody)", 100, 5];
			if (_myPlaces isEqualTo []) then {_myPlaces = [_positions];};
			...
			_waypoint0 = _group addwaypoint [_RandomLocationL,0];	
			_waypoint0 setWaypointSpeed "FULL";			
			_waypoint0 = _group addwaypoint [_RandomLocation,0];	
			_waypoint0 setWaypointSpeed "FULL";
			_group setCurrentWaypoint [_group,(_waypoint0 select 1)];
